...
if __name__ == "__main__":
	...
	parser.add_argument('--id', type=str, default='DAVE2-SNGP-GPU-c5and095-noNorm', help='Unique experiment identifier.')
	...
	def training_step(engine, batch):
		...
		#command = torch.zeros_like(batch['target_command'])
	...
	def log_losses(trainer):
		with open(os.path.join(args.logdir, "train_loss_array.txt"), "w") as txt_file:
			for line in train_loss:
				txt_file.write(" ".join(line) + "\n") 
		with open(os.path.join(args.logdir, "val_loss_array.txt"), "w") as txt_file:
