...
from DS.augment import hard as augmenter
...
class CARLA_Data(Dataset):
	...
	def __getitem__(self, index):
		...
		if self.img_aug:
			data['front_img'] = torch.Tensor(augmenter(self._batch_read_number).augment_image(np.array(
					Image.open(self.root+self.front_img[index][0])))).reshape((3, 256, 900))
		else:
			data['front_img'] = torch.Tensor(np.array(Image.open(self.root+self.front_img[index][0]))).reshape((3, 256, 900))
